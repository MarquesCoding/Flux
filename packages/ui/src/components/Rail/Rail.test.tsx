...
describe('Rail', () => {
  ...
  it('turns the page on, leaving part of a card showing as the markers do', async () => {
    const user = userEvent.setup();
    const { container } = render(<Rail title="Recently added">{items}</Rail>);
    const track = container.querySelector('ul');
    const scrollTo = vi.fn();
  ...
  it('takes no presses while it is hidden, so a tap on a phone reaches the card beneath', () => {
    const { container } = render(<Rail title="Recently added">{items}</Rail>);
    const track = container.querySelector('ul');
