...
describe('readTotpSecret', () => {
  it('extracts the secret from an otpauth uri', () => {
    expect(
      readTotpSecret(
        'otpauth://totp/Valence:admin@valence.test?secret=JBSWY3DPEHPK3PXP&issuer=Valence',
      ),
    ).toBe('JBSWY3DPEHPK3PXP');
  });

  it('returns nothing when the uri has no secret', () => {
    expect(readTotpSecret('otpauth://totp/Valence:admin@valence.test?issuer=Valence')).toBe('');
  });
