...
import createDatabaseMaintenanceServiceModule from './createDatabaseMaintenanceService'
...
describe('createDatabaseMaintenanceService', () => {
  it('queues image cache cleanup under its own kind as a singleton key', async () => {
    ...
    expect(enqueue).toHaveBeenCalledWith('server.cleanupImageCache', {}, 'server.cleanupImageCache')
