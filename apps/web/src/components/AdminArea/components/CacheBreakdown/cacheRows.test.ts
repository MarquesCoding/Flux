...
describe('cacheRows', () => {
  it('names every kind, even before anything has been counted', () => {
    expect(cacheRows(null, null, 0).map((row) => row.label)).toEqual([
      'Preview clips',
      'Scrub thumbnails',
      'Transcode sessions',
      'Artwork',
  ...
  it('says it is still counting rather than claiming an empty cache', () => {
    const rows = cacheRows(null, null, 0);
  ...
  it('reports what each kind costs', () => {
    const rows = cacheRows(cache(), { count: 40, bytes: 20 * 1024 ** 2, atMs: 1 }, 0);
  ...
  it('counts each kind in its own words', () => {
    const rows = cacheRows(cache(), { count: 40, bytes: 1, atMs: 1 }, 0);
  ...
  describe('transcode working directories', () => {
    it('reports them as running when that is what they are', () => {
      const rows = cacheRows(cache({ sessions: { count: 2, bytes: 100 } }), null, 2);
    ...
    it('does not report an abandoned directory as work in progress', () => {
      const rows = cacheRows(cache({ sessions: { count: 112, bytes: 65 * 1024 ** 3 } }), null, 1);
    ...
    it('counts what is left behind rather than what is on the disk', () => {
      const rows = cacheRows(cache({ sessions: { count: 3, bytes: 10 } }), null, 2);
    ...
    it('does not go negative when more are running than have directories', () => {
      const rows = cacheRows(cache({ sessions: { count: 1, bytes: 10 } }), null, 4);
  ...
  it('reports the kinds one service knows while the other is still counting', () => {
    const rows = cacheRows(cache(), null, 0);
  ...
  it('reports artwork while the media service is still counting', () => {
    const rows = cacheRows(null, { count: 3, bytes: 1024, atMs: 1 }, 0);
