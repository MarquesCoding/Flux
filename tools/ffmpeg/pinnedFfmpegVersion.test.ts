...
const CI_VERSION = /^\s*FLUX_FFMPEG_VERSION:\s*(?<version>\S+)\s*$/mu;

describe('pinnedFfmpegVersion', () => {
  it('reads the version the image is pinned to', () => {
    expect(pinnedFfmpegVersion('ARG FLUX_FFMPEG_VERSION=8.1.2-2flux1\n')).toBe('8.1.2-2flux1');
  });

  it('ignores a mention that is not the declaration', () => {
    const dockerfile = [
      '# FLUX_FFMPEG_VERSION=9.9.9 would be a comment, not a pin',
